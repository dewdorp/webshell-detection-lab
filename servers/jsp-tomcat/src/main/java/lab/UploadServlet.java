...
public class UploadServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("/".equals(path) || "/index.html".equals(path)) {
            serveFile(resp, new File(rootDir, "common/templates/index.html"), "text/html; charset=utf-8");
            return;
        }

        if ("/login.html".equals(path) || "/signup.html".equals(path) || "/dashboard.html".equals(path) || "/upload.html".equals(path)) {
            serveFile(resp, new File(rootDir, "common/templates/" + cleanFilename(path.substring(1))), "text/html; charset=utf-8");
            return;
        }

        if ("/upload".equals(path) && "GET".equalsIgnoreCase(req.getMethod())) {
            serveFile(resp, new File(rootDir, "common/templates/upload.html"), "text/html; charset=utf-8");
            return;
        }
        ...
        if (path.matches("^/(styles\\.css|login\\.js|signup\\.js|dashboard\\.js|upload\\.js)$")) {
            ...
            serveFile(resp, new File(rootDir, "common/public/" + name), type);
            return;
        }

        if (path.startsWith("/uploads/")) {
            String name = cleanFilename(URLDecoder.decode(path.substring("/uploads/".length()), StandardCharsets.UTF_8.name()));
            serveFile(resp, new File(uploadDir, name), "application/octet-stream");
            return;
        }
        ...
        if ("/api/signup".equals(path) && "POST".equalsIgnoreCase(req.getMethod())) {
            String body = new String(req.getInputStream().readAllBytes(), StandardCharsets.UTF_8);
            String username = extractJsonField(body, "username");
            String password = extractJsonField(body, "password");
            if (username.isBlank() || password.isBlank()) {
                json(resp, 400, "{\"success\":false,\"message\":\"Username and password are required.\"}");
                return;
            }
            json(resp, 200, "{\"success\":true,\"message\":\"Signup complete\"}");
            return;
        }

        if ("/api/login".equals(path) && "POST".equalsIgnoreCase(req.getMethod())) {
            String body = new String(req.getInputStream().readAllBytes(), StandardCharsets.UTF_8);
            String username = extractJsonField(body, "username");
            String password = extractJsonField(body, "password");
            if (username.isBlank() || password.isBlank()) {
            ...
            json(resp, 200, "{\"success\":true,\"token\":\"" + tokenFor(username) + "\"}");
            return;
        }

        if ("/api/dashboard".equals(path)) {
            String username = usernameFromToken(req.getHeader("Authorization"));
            if (username.isBlank()) {
                json(resp, 401, "{\"success\":false,\"message\":\"Authentication required\"}");
                return;
            }
            json(resp, 200, "{\"success\":true,\"username\":\"" + escape(username) + "\"}");
            return;
        }

        if ("/upload".equals(path) && "POST".equalsIgnoreCase(req.getMethod())) {
    ...
    private void handleUpload(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        ...
        if (part == null || part.getSubmittedFileName() == null) {
            json(resp, 400, "{\"success\":false,\"message\":\"file field is required\"}");
            return;
        }
        ...
        String event = "{"
            ...
            + "\"storedName\":\"" + escape(storedName) + "\","
            + "\"storedPath\":\"" + escape(storedFile.getAbsolutePath()) + "\","
            + "\"size\":" + storedFile.length() + ","
            + "\"mimeType\":\"" + escape(part.getContentType() == null ? "application/octet-stream" : part.getContentType()) + "\","
            + "\"sha256\":\"" + sha256(storedFile) + "\","
            + "\"uploadedAt\":\"" + Instant.now().toString() + "\","
            + "\"remoteAddress\":\"" + escape(req.getRemoteAddr()) + "\""
            + "}";
    ...
    private void listFiles(HttpServletResponse resp) throws IOException {
        ...
        for (int i = 0; i < files.size(); i++) {
            ...
            if (i > 0) body.append(",");
            body.append("{")
                .append("\"name\":\"").append(escape(file.getName())).append("\",")
                .append("\"size\":").append(file.length()).append(",")
                .append("\"modifiedAt\":\"").append(Instant.ofEpochMilli(file.lastModified()).toString()).append("\",")
                .append("\"url\":\"/uploads/").append(escape(file.getName())).append("\"")
                .append("}");
        }
        body.append("]}");
    ...
    private String cleanFilename(String filename) {
        String name = new File(filename == null || filename.isEmpty() ? "upload.bin" : filename).getName();
    ...
    private String sha256(File file) throws IOException {
        try {
            ...
            throw new IOException("failed to hash file", error);
        }
    }
    ...
    private String tokenFor(String username) {
        String payload = "{\"username\":\"" + escape(username) + "\",\"issuedAt\":" + System.currentTimeMillis() + "}";
    ...
    private String usernameFromToken(String authHeader) {
        ...
        try {
            ...
            return extractJsonField(payload, "username");
        } catch (Exception error) {
            return "";
        }
    }
