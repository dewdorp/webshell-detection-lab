...
need_root() {
  if [ "$(id -u)" -ne 0 ]; then
    printf 'Nginx upstream update needs root. Run switch-server.sh with sudo or set LAB_AUTO_NGINX_UPSTREAM=0.\n' >&2
    exit 1
  fi
}
...
update_site() {
  ...
  if cmp -s "$NGINX_AVAILABLE" "$tmp"; then
    rm -f "$tmp"
    printf 'Nginx lab upstream is already http://%s:%s\n' "$LAB_UPSTREAM_HOST" "$LAB_UPSTREAM_PORT"
    return
  fi
  ...
  reload_nginx
  printf 'Nginx lab upstream updated to http://%s:%s\n' "$LAB_UPSTREAM_HOST" "$LAB_UPSTREAM_PORT"
...
if [ ! -f "$NGINX_AVAILABLE" ]; then
  printf 'Nginx site config not found at %s; skipping lab upstream update.\n' "$NGINX_AVAILABLE"
  exit 0
fi
